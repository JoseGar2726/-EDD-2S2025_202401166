...
begin
  ...
  // Grafo dirigido con flechas en ambos extremos
  Writeln(f, 'digraph G {');
  Writeln(f, '  node [shape=circle, style=filled, fillcolor=lightblue];');
  Writeln(f, '  rankdir=LR; // disposición izquierda-derecha opcional');
  ...
  begin
    ...
    begin
      if (i < temp^.vertice) then
        Writeln(f, '  ', i, ' -> ', temp^.vertice, ' [dir=both];');
      temp := temp^.siguiente;
    end;
  end;
  ...
  if ExecuteProcess('cmd.exe', ['/C', comando]) = 0 then
