 ...
 function SearchBST(var root: PComunidadHeader; nameCom: String): PComunidadHeader;
  ...
  procedure GenerarNodosDOT(var SL: TStringList; nodo: PComunidadHeader);
   ...
   begin
       ...
       SL.Add(Format('"%s" [label="Comunidad: %s\nFecha: %s\nMensajes: %d"];',
       ...
       while u <> nil do
       begin
           SL.Add(Format('"U_%s_%s" [label="Usuario: %s", shape=note, color=lightblue];',
               [nodo^.nombreComunidad, u^.correo, u^.correo]));
           SL.Add(Format('"%s" -> "U_%s_%s";',
               [nodo^.nombreComunidad, nodo^.nombreComunidad, u^.correo]));
           u := u^.next;
       end;
       ...
       while m <> nil do
       begin
           SL.Add(Format('"M_%s_%s" [label="De: %s\n%s\n%s", shape=box, color=yellow];',
               [nodo^.nombreComunidad, m^.correo, m^.correo, m^.mensaje, m^.fecha]));
           SL.Add(Format('"%s" -> "M_%s_%s";',
               [nodo^.nombreComunidad, nodo^.nombreComunidad, m^.correo]));
           m := m^.next;
       end;
